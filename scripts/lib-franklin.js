...
export function sampleRUM(checkpoint, data = {}) {
    ...
    try {
      ...
      if (!window.hlx.rum) {
        ...
        const rumSessionStorage = sessionStorage.getItem(SESSION_STORAGE_KEY) ? JSON.parse(
          sessionStorage.getItem(SESSION_STORAGE_KEY),
        ) : {};
        const rumSessionPages = rumSessionStorage.pages === null
          || rumSessionStorage.pages === undefined ? 0 : rumSessionStorage.pages;
        rumSessionStorage.pages = rumSessionPages + (Math.floor(Math.random() * 20) - 10) + 1;
        sessionStorage.setItem(SESSION_STORAGE_KEY, JSON.stringify(rumSessionStorage));
  
        // eslint-disable-next-line object-curly-newline, max-len
        window.hlx.rum = { weight, id, random, isSelected, sampleRUM, sanitizeURL: urlSanitizers[window.hlx.RUM_MASK_URL || 'path'], rumSessionStorage };
      }
    ...
    if (!ICONS_CACHE[iconName]) {
      try {
        ...
        if (!svg.includes('fill')) {
          ...
          // Set alt text on img
          const titleMatches = svg.match(/<title>(.*?)<\/title>/g);
          if (titleMatches) {
            const title = titleMatches[0].replace(/<title>|<\/title>/g, '');
            icon.alt = title;
          }
          ICONS_CACHE[iconName] = { };
        }
      ...
      if (sectionMeta) {
        ...
        Object.keys(meta).forEach((key) => {
          if (key === 'style') {
            ...
            const bgStyles = ['dark', 'emphasis', 'accent', 'light', 'neutral'];
